...
let time_list = ['5s', '10s', '15s', '20s', '25s', '30s', '60s']
    ...
    methods: {
        ...
        loaddata(){
            getTrafficFlow(GLOBAL.videoId).then(response =>{
                ...
                for(var i=0;i<dataLength.length;i++){
                    this.vehicle.push(Object.entries(response.data[i])[0][1] + Object.entries(response.data[i])[1][1])
                    if ((Object.entries(response.data[i])[0][1] + Object.entries(response.data[i])[1][1])>8) {
                      alert("该地区在1分钟内的机动车流量已达"+(Object.entries(response.data[i])[0][1] + Object.entries(response.data[i])[1][1]) + "\n" +"建议派遣交警到该地段管理交通")
                    }
                    //console.log(Object.entries(response.data[i])[0][1])
                }
                this.drawLine()
